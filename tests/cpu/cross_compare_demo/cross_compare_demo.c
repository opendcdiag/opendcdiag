...
static int custom_memcmp(void* actual, void* expected, size_t size) {
    uint8_t* cursor_actual = (uint8_t*) actual;
    uint8_t* cursor_expected = (uint8_t*) expected;
    for (int i = 0; i < (int)size; ++i) {
        if((cursor_actual[i] ^ cursor_expected[i]) != 0) {
            return cursor_actual[i] ^ cursor_expected[i];
        }
    }
...
static int run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 1) {
        ...
        cross_compare_or_fail(test, "golden", dst, NUM_ELEMENTS);
        if (cross_compare(test, "golden", dst, NUM_ELEMENTS, custom_memcmp) != 0) {
            log_error("Cross compare with custom memcmp failed!");
            return EXIT_FAILURE;
        }
    }
