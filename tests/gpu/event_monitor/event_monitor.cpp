...
void listener_runner(test_data* td)
{
    ...
    while (td->thread_data.thread_should_run.load(std::memory_order::relaxed)) {
...
int listener_preinit(struct test* test)
{
    ...
    try {
        td->thread = std::thread(listener_runner, td.get());
    } catch (...) {
...
int listener_postcleanup(struct test* test)
{
    ...
    td->thread_data.thread_should_run.store(false, std::memory_order::relaxed);
    td->thread.join();
    ...
    if (any_event) {
        for (size_t i = 0; i < td->thread_data.devices.size(); i++) {
            if (td->thread_data.n_events[i] > 0) {
                log_message(int(i), "GPU %zu: %" PRIu64 " event notifications, mask: %#x",
...
DECLARE_MANUAL_TEST(event_monitor, "Monitor GPU device events for hardware issues")
    ...
    .flags = test_in_parent | test_schedule_sequential,
END_DECLARE_TEST
