...
protected:
    ...
    C* get_random_bits_cache(thread_rng* rng) {
        static_assert(sizeof(rng_context) <= sizeof(*rng), "RNG context must fit thread state");
        static_assert(sizeof(C) <= sizeof(rng_context::cache), "Cache must fit in the RNG context field");
