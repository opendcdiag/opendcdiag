...
void append_xe_devcoredump(std::string &out, const gpu_info_t &info)
{
    ...
    struct stat st = {};
    if (stat(devcoredump_data.c_str(), &st) != 0) {
        return;
    }
    ...
    char base[sizeof "2147483647-12-31T23:59:60.999999999Z"];
    size_t off = strftime(base, sizeof(base), "%Y-%m-%dT%H:%M:%S", &tm);
    std::string ts(base, off);
    ts += std::format(".{:09}Z", st.st_mtim.tv_nsec);

    out += std::format("  xe_devcoredump_path: {} (mtime: {})\n",
                       devcoredump_data.string(), ts);
}
