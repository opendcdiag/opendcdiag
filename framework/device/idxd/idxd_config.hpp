...
struct idxd_config_t
{
    ...
    struct config_t
    {
        ...
        struct wq_t
        {
            ...
            std::optional<unsigned int> max_batch_size;
            std::optional<uint64_t> max_transfer_size;
            int ats_disable = -1;
            int prs_disable = -1;
        ...
        // Template applied to every object of the matching device type found on the system.
        struct template_t
        {
            accfg_device_type device_type = ACCFG_DEVICE_TYPE_UNKNOWN;  // UNKNOWN matches any type
            std::optional<device_t> device;
            std::optional<group_t> group;
            std::optional<engine_t> engine;
            std::optional<wq_t> wq;
        };
...
// UniqueTag makes every macro expansion to emit its own function, and therefore own static.
// Without it all tests would share one config.
template <typename UniqueTag>
inline idxd_config_t* make_static_idxd_config(const idxd_config_t& config)
{
    static idxd_config_t stored = config;
    return &stored;
}
...
// decltype([]{}) is the trick to emit unique closure type per macro expansion, because each
// lambda produces its own unique unnamed type.
#define DECLARE_TEST_IDXD_CONFIG(...) \
    make_static_idxd_config<decltype([]{})>({ __VA_ARGS__ })
