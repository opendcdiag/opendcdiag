...
int write_config(const idxd_config_t::config_t& from)
{
    ...
    for (const auto& g : from.groups) {
        ...
        if (g.use_read_buffer_limit >= 0 &&
            ...
            accfg_group_set_use_read_buffer_limit(group, g.use_read_buffer_limit) < 0) {
        ...
        if (g.traffic_class_a >= 0 &&
            ...
            accfg_group_set_traffic_class_a(group, g.traffic_class_a) < 0) {
        ...
        if (g.traffic_class_b >= 0 &&
            ...
            accfg_group_set_traffic_class_b(group, g.traffic_class_b) < 0) {
    ...
    for (const auto& q : from.wqs) {
        ...
        if (q.max_batch_size) {
            ...
            if (ret < 0) {
                log_error("Failed to set work queue max batch size to %u: %s",
                          *q.max_batch_size, strerror(-ret));
                return EXIT_FAILURE;
            }
        }
        if (q.max_transfer_size) {
            ...
            if (ret < 0) {
                log_error("Failed to set work queue max transfer size to %lu: %s",
                          *q.max_transfer_size, strerror(-ret));
                return EXIT_FAILURE;
            }
        }
        ...
        if (q.ats_disable >= 0) {
            ...
            if (current != q.ats_disable && accfg_wq_set_ats_disable(wq, q.ats_disable) < 0) {
        ...
        if (q.prs_disable >= 0) {
            ...
            if (current != q.prs_disable && accfg_wq_set_prs_disable(wq, q.prs_disable) < 0) {
