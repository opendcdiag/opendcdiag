...
struct ProgramOptionsParser {
    ...
    int collect_args(int argc, char** argv)
    {
        ...
        while ((opt = simple_getopt(argc, argv, long_options, &coptind)) != -1) {
            ...
            case alpha_option:
                ...
                opts_map.erase(beta_option);
                opts_map.erase(quality_option);
                break;
            case beta_option:
                opts_map.emplace(beta_option, true);

                opts_map.erase(alpha_option);
                opts_map.erase(quality_option);
                break;
            case quality_option:
            ...
            case 'T':
                ...
                opts_map.erase(one_sec_option);
                opts_map.erase(thirty_sec_option);
                opts_map.erase(two_min_option);
                opts_map.erase(five_min_option);
                break;
            case 'v':
                if (!opts_map.count('v')) {
    ...
    int validate_args() const
    {
        if (opts_map.count(one_sec_option) + opts_map.count(thirty_sec_option) + opts_map.count(five_min_option) + opts_map.count('T') >= 2) {
            fprintf(stdout, "Warning: At least 2 options from conflicting set [1sec, 30sec, 2min, 5min, total-time] given, using the last specified one\n");
        }
        ...
        if (opts_map.count(alpha_option) + opts_map.count(beta_option) + opts_map.count(quality_option) + opts_map.count(selftest_option) >= 2) {
            fprintf(stdout, "Warning: At least 2 options from conflicting sert [alpha, beta, quality, selftests] given, using the last specified one\n");
        }
#else
        ...
        if (opts_map.count('T') && opts_map.count(service_option)) {
            fprintf(stdout, "Warning: At least 2 options from conflicting sert [total-time, service] given, using the last specified one\n");
        }
        ...
        if (opts_map.count(quick_run_option) && opts_map.count(test_delay_option)) {
            fprintf(stdout, "Warning: At least 2 options from conflicting sert [quick, test-delay] given, using the last specified one\n");
        }
