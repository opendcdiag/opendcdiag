...
struct ProgramOptionsParser {
    ...
    int collect_args(int argc, char** argv)
    {
        ...
        while ((opt = simple_getopt(argc, argv, long_options, &coptind)) != -1) {
            ...
            case inject_idle_option:
                opts_map.emplace(inject_idle_option, optarg);
                break;
            case is_asan_option:
                // these options are only accessible in the command-line if the
                // corresponding functionality is active
                opts_map.insert_or_assign(_action_option, is_asan_option);
                opts_map.emplace(is_asan_option, true);
                break;
            case is_debug_option:
                // these options are only accessible in the command-line if the
                // corresponding functionality is active
                opts_map.insert_or_assign(_action_option, is_debug_option);
            ...
            // XXX For legacy reasons, we allow those conflicting opts and take only the last specified one
            case alpha_option:
                opts_map.insert_or_assign(_quality_option, alpha_option);
                opts_map.emplace(alpha_option, true);
                break;
            case beta_option:
                opts_map.insert_or_assign(_quality_option, beta_option);
                opts_map.emplace(beta_option, true);
                break;
            case quality_option:
                opts_map.insert_or_assign(_quality_option, quality_option);
                opts_map.insert_or_assign(quality_option, optarg);
                break;

            case 'l':
                opts_map.insert_or_assign(_action_option, 'l');
                opts_map.emplace('l', true);
                break;
            case raw_list_tests:
                opts_map.insert_or_assign(_action_option, raw_list_tests);
                opts_map.emplace(raw_list_tests, true);
                break;
            case raw_list_groups:
                opts_map.insert_or_assign(_action_option, raw_list_groups);
            ...
            case dump_cpu_info_option:
                opts_map.insert_or_assign(_action_option, dump_cpu_info_option);
                opts_map.emplace(dump_cpu_info_option, true);
                break;
            case version_option:
                opts_map.insert_or_assign(_action_option, version_option);
                opts_map.emplace(version_option, true);
                break;
            case 'h':
                opts_map.insert_or_assign(_action_option, 'h');
                opts_map.emplace('h', true);
                break;

            case service_option:
                opts_map.insert_or_assign(_duration_option, service_option);
            ...
            case 'T':
                ...
                break;
            case one_sec_option:
                opts_map.insert_or_assign(_duration_option, one_sec_option);
                opts_map.emplace(one_sec_option, true);
                break;
            case thirty_sec_option:
                opts_map.insert_or_assign(_duration_option, thirty_sec_option);
                opts_map.emplace(thirty_sec_option, true);
                break;
            case two_min_option:
                opts_map.insert_or_assign(_duration_option, two_min_option);
                opts_map.emplace(two_min_option, true);
                break;
            case five_min_option:
                opts_map.insert_or_assign(_duration_option, five_min_option);
    ...
    // To not make it a breaking change, we print a benign warning message and continue execution.
    int validate_args() const
    {
        if (opts_map.count(one_sec_option) + opts_map.count(thirty_sec_option) +
            opts_map.count(two_min_option) + opts_map.count(five_min_option) +
            opts_map.count('T') + opts_map.count(service_option) >= 2) {
            fprintf(stdout, "Warning: At least 2 options from conflicting set [1sec, 30sec, 2min, 5min, total-time, service] given, using the last specified one\n");
        }

        if (opts_map.count(raw_list_tests) + opts_map.count(raw_list_groups) +
            opts_map.count(raw_list_group_members) + opts_map.count(dump_cpu_info_option) + opts_map.count(version_option) >= 2) {
            fprintf(stdout, "Warning: At least 2 options from conflicting set [list-tests, list-groups, list-group-members, dump-cpu-info, version] given, using the last specified one\n");
        }

        if (opts_map.count(alpha_option) + opts_map.count(beta_option) + opts_map.count(quality_option) >= 2) {
            fprintf(stdout, "Warning: At least 2 options from conflicting sert [alpha, beta, quality] given, using the last specified one\n");
        }

        if (opts_map.count(max_cores_per_slice_option) && opts_map.count(no_slicing_option)) {
            fprintf(stdout, "Warning: At least 2 options from conflicting sert [no-slicing, max-cores-per-slice] given, using the last specified one\n");
        }
        ...
        if (opts_map.count(output_format_option) && opts_map.count('Y')) {
            if (strcmp(std::get<const char*>(opts_map.at(output_format_option)), "yaml")) {
                fprintf(stdout, "Warning: At least 2 options from conflicting sert [yaml, output-format] given, using the last specified one\n");
            }
        }

        if (opts_map.count('q') && opts_map.count('v')) {
            fprintf(stdout, "Warning: At least 2 options from conflicting sert [quiet, verbose] given, using the last specified one\n");
        }

        if (opts_map.count(quick_run_option) && opts_map.count(max_test_loop_count_option)) {
            fprintf(stdout, "Warning: At least 2 options from conflicting sert [quick, max-test-loop-count] given, using the last specified one\n");
        }
    ...
    int parse_args(SandstoneApplication* app, ProgramOptions& opts, char** argv)
    {
        ...
        if (opts_map.count(_quality_option)) {
            switch (std::get<int>(opts_map[_quality_option])) {
            case alpha_option:
        ...
        // endpoints
        if (opts_map.count(_action_option)) {
            switch (std::get<int>(opts_map[_action_option])) {
            case is_asan_option:
            case is_debug_option:
        ...
        if (opts_map.count(_duration_option)) {
            switch (std::get<int>(opts_map[_duration_option])) {
            case 'T': {
        ...
        if (opts_map.count(_format_option)) {
            switch (std::get<int>(opts_map[_format_option])) {
            case 'Y': {
        ...
        if (opts_map.count(_max_cores_option)) {
            switch (std::get<int>(opts_map[_max_cores_option])) {
            case max_cores_per_slice_option:
        ...
        if (opts_map.count(_max_loop_count_option)) {
            switch (std::get<int>(opts_map[_max_loop_count_option])) {
            case max_test_loop_count_option:
        ...
        if (opts_map.count(inject_idle_option)) {
            app->inject_idle = ParseIntArgument<>{
                .name = "--inject-idle",
                .min = 0,
                .max = 50,
                .range_mode = OutOfRangeMode::Saturate
            }(std::get<const char*>(opts_map.at(inject_idle_option)));
        }
