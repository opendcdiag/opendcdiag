...
struct CmdlineParser {
    ...
    int validate_args() const
    {
        ...
        if (args_present.quick_run && args_present.max_test_loop_count) {
            throw std::invalid_argument{"Options quick, max-test-loop-count are mutually exclusive"};
        }

        if (args_present.quick_run && args_present.delay_between_tests) {
            throw std::invalid_argument{"Options quick, test-delay are mutually exclusive"};
        }
    ...
    int parse_args(SandstoneApplication* app, ParsedCmdLineOpts& opts, char** argv)
    {
        ...
        if (args_present.list) {
            ...
            // return EXIT_SUCCESS; // do we exit immediately or proceed further?
        }
    ...
    int parse_restricted_command_line(int argc, char** argv, SandstoneApplication* app, ParsedCmdLineOpts& opts) {
...
int ParsedCmdLineOpts::parse(int argc, char** argv, SandstoneApplication* app, ParsedCmdLineOpts& opts) {
