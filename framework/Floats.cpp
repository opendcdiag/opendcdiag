...
static_assert((FP_NORMAL & (~FP_RANDOM_GEN_FLAGS_CMATH_MASK)) == 0, "FP_NORMAL value collides with random generator flags");
static_assert((FP_NORMAL & FP_RANDOM_GEN_FLAGS_CMATH_MASK) != 0, "FP_NORMAL value cannot be used as a selector");

static_assert((FP_SUBNORMAL & (~FP_RANDOM_GEN_FLAGS_CMATH_MASK)) == 0, "FP_SUBNORMAL value collides with random generator flags");
static_assert((FP_SUBNORMAL & FP_RANDOM_GEN_FLAGS_CMATH_MASK) != 0, "FP_SUBNORMAL value cannot be used as a selector");

static_assert((FP_ZERO & (~FP_RANDOM_GEN_FLAGS_CMATH_MASK)) == 0, "FP_ZERO value collides with random generator flags");
static_assert((FP_ZERO & FP_RANDOM_GEN_FLAGS_CMATH_MASK) != 0, "FP_ZERO value cannot be used as a selector");

static_assert((FP_INFINITE & (~FP_RANDOM_GEN_FLAGS_CMATH_MASK)) == 0, "FP_INF value collides with random generator flags");
static_assert((FP_INFINITE & FP_RANDOM_GEN_FLAGS_CMATH_MASK) != 0, "FP_INF value cannot be used as a selector");

#if FP_NAN != 0
static_assert((FP_NAN & (~FP_RANDOM_GEN_FLAGS_CMATH_MASK)) == 0, "FP_NAN value collides with random generator flags");
static_assert((FP_NAN & FP_RANDOM_GEN_FLAGS_CMATH_MASK) != 0, "FP_NAN value cannot be used as a selector");
#else
#warning "FP_NAN value is zero, cannot be used as a selector in random float generator!"
#endif
...
F gen_random_impl(uint32_t flags) {
    ...
    if (flags == FP_RANDOM_GEN_FLAGS_FAST_ZERO) {
        f = F{ 0, 0, 0 };
        return f;
    }
    if (flags == FP_RANDOM_GEN_FLAGS_FAST_MEMSET) {
        random_fast_memset_impl<F>(f);
        return f;
    }
    if (flags == FP_RANDOM_GEN_FLAGS_OBSOLETE_GENERATOR) {
        f = F{
    ...
    assert(({
            constexpr uint32_t handled =
                FP_RANDOM_GEN_FLAGS_CMATH_MASK |
                FP_RANDOM_GEN_FLAGS_MANTISSA_MASK | FP_RANDOM_GEN_FLAGS_EXPONENT_MASK | FP_RANDOM_GEN_FLAGS_SIGN_MASK |
                FP_RANDOM_GEN_FLAGS_FORCE_FINITE | FP_RANDOM_GEN_FLAGS_NO_SUBNORMALS;
            ((flags & (~handled)) == 0);
        }) && "Unknown flags in random float generator");

    // lib math selectors
    switch (flags & FP_RANDOM_GEN_FLAGS_CMATH_MASK) {
        case 0:
            // no FP selected
            break;
        case FP_NORMAL:
            flags |= (FP_RANDOM_GEN_FLAGS_FORCE_FINITE | FP_RANDOM_GEN_FLAGS_NO_SUBNORMALS);
            break;
        case FP_ZERO:
            flags = (flags & (~FP_RANDOM_GEN_FLAGS_EXPONENT_MASK)) | FP_RANDOM_GEN_FLAGS_VALUE_ZERO;
            break;
        case FP_SUBNORMAL:
            flags = (flags & (~FP_RANDOM_GEN_FLAGS_EXPONENT_MASK)) | FP_RANDOM_GEN_FLAGS_VALUE_DENORMAL;
            break;
        case FP_INFINITE:
            flags = (flags & (~FP_RANDOM_GEN_FLAGS_EXPONENT_MASK)) | FP_RANDOM_GEN_FLAGS_VALUE_INF;
            break;
        #if FP_NAN != 0
        case FP_NAN:
            // currently FP_NAN is 0.. cannot be used as a selector..
            flags = (flags & (~FP_RANDOM_GEN_FLAGS_EXPONENT_MASK)) | FP_RANDOM_GEN_FLAGS_VALUE_NAN;
            break;
        #endif
        default:
            assert(false && "Unhandled libmath FP class in random float generator");
            // just ignore and continue with other flag groups
    }

    switch (flags & FP_RANDOM_GEN_FLAGS_SIGN_MASK) {
        case FP_RANDOM_GEN_FLAGS_SIGN_POSITIVE:
            f.sign = 0;
            break;
        case FP_RANDOM_GEN_FLAGS_SIGN_NEGATIVE:
            f.sign = 1;
            break;
        case FP_RANDOM_GEN_FLAGS_SIGN_BITS:
            f.sign = get_random_bits<BaseType>(1);
            break;
        case FP_RANDOM_GEN_FLAGS_SIGN_RANDOM:
            f.sign = random32();
            break;
        default:
    ...
    switch (flags & FP_RANDOM_GEN_FLAGS_EXPONENT_MASK) {
        case FP_RANDOM_GEN_FLAGS_EXPONENT_RANDOM:
            f.exponent = random32();
            break;
        case FP_RANDOM_GEN_FLAGS_EXPONENT_BITS:
            // flat distribution only
            f.exponent = get_random_bits<BaseType>(F::exponent_bits());
            break;
        case FP_RANDOM_GEN_FLAGS_EXPONENT_GAUSSIAN2:
        ...
        case FP_RANDOM_GEN_FLAGS_EXPONENT_GAUSSIAN8:
            ...
            f.exponent = get_random_bits<BaseType>(F::exponent_bits() - 1) +
                            ...
                            get_random_bits<BaseType>(F::exponent_bits() - 3);
            break;
        case FP_RANDOM_GEN_FLAGS_EXPONENT_VECTOR:
            f.exponent = get_predefined_float<F>(selected_predefined).exponent;
            break;

        case FP_RANDOM_GEN_FLAGS_EXPONENT_MAX:
            // in most cases (except HFloat8) infinity, NaN or overflow
            f.exponent = get_mask<BaseType>(F::exponent_bits());
            break;
        case FP_RANDOM_GEN_FLAGS_EXPONENT_BIAS:
            // range [1..2)
            f.exponent = get_exponent_bias<F>();
            break;

        case FP_RANDOM_GEN_FLAGS_EXPONENT_ZERO:
        case FP_RANDOM_GEN_FLAGS_VALUE_ZERO:
        case FP_RANDOM_GEN_FLAGS_VALUE_DENORMAL:
            // zero or denormal
            f.exponent = 0;
            break;
        case FP_RANDOM_GEN_FLAGS_VALUE_INF:
        case FP_RANDOM_GEN_FLAGS_VALUE_OVERFLOW:
            // do not care about the mantissa
            flags |= FP_RANDOM_GEN_FLAGS_MANTISSA_ZERO;
            break;
        case FP_RANDOM_GEN_FLAGS_VALUE_NAN:
        case FP_RANDOM_GEN_FLAGS_VALUE_SNAN:
        case FP_RANDOM_GEN_FLAGS_VALUE_QNAN:
            // exponent will be set later (enforced)
            break;
    ...
    switch (flags & FP_RANDOM_GEN_FLAGS_MANTISSA_MASK) {
        case FP_RANDOM_GEN_FLAGS_MANTISSA_ZERO:
            f.mantissa = 0;
            break;
        case FP_RANDOM_GEN_FLAGS_MANTISSA_BITS:
            // flat distribution only
            f.mantissa = get_random_bits<BaseType>(F::mantissa_bits());
            break;
        case FP_RANDOM_GEN_FLAGS_MANTISSA_PATTERNED:
            // existing pattern to generate "chains" of 1s and 0s
            f.mantissa = set_random_bits(random32() % (F::mantissa_bits() + 1), F::mantissa_bits());
            break;
        case FP_RANDOM_GEN_FLAGS_MANTISSA_RANDOM:
            ...
            break;
        case FP_RANDOM_GEN_FLAGS_MANTISSA_VECTOR:
    ...
    if (flags & FP_RANDOM_GEN_FLAGS_FORCE_FINITE) {
        assert(({
                bool compatible;
                switch (flags & FP_RANDOM_GEN_FLAGS_EXPONENT_MASK) {
                    case FP_RANDOM_GEN_FLAGS_VALUE_INF:
    ...
    if (flags & FP_RANDOM_GEN_FLAGS_NO_SUBNORMALS) {
        assert(({
                bool compatible;
                switch (flags & FP_RANDOM_GEN_FLAGS_EXPONENT_MASK) {
                    case FP_RANDOM_GEN_FLAGS_VALUE_ZERO:
...
F normalize_impl(F f, float v1, float v2, uint32_t flags) {
    ...
    assert(!(((s1 == Z) || f1.is_negative()) && ((s2 == Z) || f2.is_negative()) &&
             ((flags & FP_RANDOM_GEN_FLAGS_SIGN_MASK) == FP_RANDOM_GEN_FLAGS_SIGN_POSITIVE) && "Negative range for positive value"));
    assert(!(((s1 == Z) || (!f1.is_negative())) && ((s2 == Z) || (!f2.is_negative())) &&
             ((flags & FP_RANDOM_GEN_FLAGS_SIGN_MASK) == FP_RANDOM_GEN_FLAGS_SIGN_NEGATIVE) && "Positive range for negative value"));
