...
template<typename F>
static inline F gen_random_impl(uint32_t flags) {
    using BaseType = F::base_type;

    if (flags & FP_GEN_PCT_VEC_SELECTOR) {
        // percentage must be in the range 1..100 to have effect
        assert(((flags & FP_GEN_PCT_VEC_SELECTOR_VAL_MASK) <= 100) && "Invalid percentage for vector selector");
        if (((flags & FP_GEN_PCT_VEC_SELECTOR_VAL_MASK) != 0) && (get_random_value<uint32_t>(100) < (flags & FP_GEN_PCT_VEC_SELECTOR_VAL_MASK))) {
            // force predefined vector value
            flags = FP_GEN_STATIC_VECTOR;
        } else {
            // generate random value as usual, discard PCT flags and generate it with other passed flags
            flags &= ~(FP_GEN_PCT_VEC_SELECTOR | FP_GEN_PCT_VEC_SELECTOR_VAL_MASK);
        }
    }

    // extra scenarios, fast paths
    switch (flags) {
