...
thread_local uint32_t random_bits_value = 0;
thread_local int random_bits_available = 0;
...
I get_random_bits(int bits) {
    if (bits > sizeof(I) * 8) {
        assert(!"Number of bits less than or equal to sizeof(I) * 8");
    }
...
thread_local uint32_t random_val_value = 0;
thread_local uint32_t random_val_available = 0;
...
F get_predefined_float(int& selected_predefined) {
    assert(!"Predefined values available for the type");
    return {};
}
...
HFloat8 force_nan(HFloat8& f, bool quiet) {
    ...
    assert(!"HFloat8 with S/Q NaN values");
    return f;
}
template<>
HFloat8 force_nan(HFloat8& f) {
    assert(!"HFloat8 with NaN values");
    return f;
}
template<>
...
F force_overflow(F& f) {
    assert(!"Specific overflow value for the type");
    return f;
}
template<>
...
F gen_random_impl(uint32_t flags) {
    ...
    switch (flags & RANDOM_GEN_FLAGS_FLOAT_EXPONENT_MASK) {
        ...
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_INF:
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_OVERFLOW:
    ...
    switch (flags & RANDOM_GEN_FLAGS_FLOAT_MANTISSA_MASK) {
        case RANDOM_GEN_FLAGS_FLOAT_MANTISSA_BITS:
            ...
            f.mantissa = get_random_bits<typename F::base_type>(F::mantissa_bits());
            break;
        ...
        case RANDOM_GEN_FLAGS_FLOAT_MANTISSA_ZERO:
            ...
            break;
        default:
            assert(!"Handled mantissa generator");
            return {};
    }
    ...
    switch (flags & RANDOM_GEN_FLAGS_FLOAT_EXPONENT_MASK) {
        ...
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_BITS:
            // flat distribution only
            f.exponent = get_random_bits<typename F::base_type>(F::exponent_bits());
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_GAUSSIAN2:
            ...
            f.exponent = get_random_bits<typename F::base_type>(F::exponent_bits() - 1) +
                       get_random_bits<typename F::base_type>(F::exponent_bits() - 1);
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_GAUSSIAN4:
            ...
            f.exponent = get_random_bits<typename F::base_type>(F::exponent_bits() - 1) +
                       get_random_bits<typename F::base_type>(F::exponent_bits() - 2) +
                       get_random_bits<typename F::base_type>(F::exponent_bits() - 2);
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_GAUSSIAN8:
            ...
            f.exponent = get_random_bits<typename F::base_type>(F::exponent_bits() - 1) +
                       get_random_bits<typename F::base_type>(F::exponent_bits() - 2) +
                       get_random_bits<typename F::base_type>(F::exponent_bits() - 3) +
                       get_random_bits<typename F::base_type>(F::exponent_bits() - 3);
            break;
        ...
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_ZERO:
            f.exponent = 0;
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_DENORMAL:
            force_denormal(f);
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_INF:
            force_infinity(f);
        ...
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_OVERFLOW:
            force_overflow(f);
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_NAN:
            force_nan(f);
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_SNAN:
            force_nan(f, false);
            break;
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_QNAN:
            force_nan(f, true);
            break;
        ...
        case RANDOM_GEN_FLAGS_FLOAT_EXPONENT_BIAS:
            ...
            break;
        default:
            assert(!"Handled exponent generator");
            return {};
    }
    ...
    switch (flags & RANDOM_GEN_FLAGS_FLOAT_SIGN_MASK) {
        ...
        case RANDOM_GEN_FLAGS_FLOAT_SIGN_BITS:
            f.sign = get_random_bits<typename F::base_type>(1);
            break;
        case RANDOM_GEN_FLAGS_FLOAT_SIGN_RANDOM:
            f.sign = random32();
            break;
        default:
            assert(!"Handled sign generator");
            return {};
    }
    return fix_bits(f);
}
...
template<typename F>
F normalize_impl(F f, float v1, float v2) {
    // Normalization should be not allowed for Inf/NaN values
    // TODO add handling for special values (e.g. v1 is a payload for NaN)

    // force [1..2), the distribution is uniform only if the source is uniform!
    // Mantissa bits might be lost if the range is big. Best results are when
    // both v1 and v2 are power of two values.
    f.sign = 0;
    f.exponent = get_exponent_bias<F>();
    auto v = f.to_float();
    // normalize it to given range. v1 is included, v2 is "open"
...
HFloat8 normalize_hfloat8(HFloat8 val, float v1, float v2) {
    return normalize_impl(val, v1, v2);
}
