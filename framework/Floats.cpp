...
F gen_random_impl(uint32_t flags) {
    ...
    if (flags & FP_GEN_PCT_VEC_SELECTOR) {
        ...
        assert(((flags & FP_GEN_PCT_VEC_SELECTOR_MASK) <= 100) && "Invalid percentage for vector selector");
        if (((flags & FP_GEN_PCT_VEC_SELECTOR_MASK) != 0) && (get_random_value<uint32_t>(100) < (flags & FP_GEN_PCT_VEC_SELECTOR_MASK))) {
            ...
            flags &= ~(FP_GEN_PCT_VEC_SELECTOR | FP_GEN_PCT_VEC_SELECTOR_MASK);
        }
    }
