...
struct ParsedCmdLineOpts {
    ...
    int parse(int argc, char** argv, SandstoneApplication* app, ParsedCmdLineOpts& opts);

    // for RestrictedCommandLine make it visible to the compiler
...
inline int parse_cmdline(int argc, char** argv, SandstoneApplication* app, ParsedCmdLineOpts& opts) {
    auto ret = opts.parse(argc, argv, app, opts);
    if constexpr (SandstoneConfig::RestrictedCommandLine) {
        opts.apply_restrictions();
    }
    return ret;
}
