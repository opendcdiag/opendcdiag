...
T get_random_bits(uint32_t bits) {
    ...
    static_assert(B <= 8 * sizeof(random_bits_value), "Generator cannot provide more bits than the type can hold");
    ...
    while (bits != 0) {
        ...
        // Lets have "undefined behaviour" if wrong number of bits is requested
...
T get_random_value(T range) {
    ...
    static_assert(B <= 8 * sizeof(random_val_value), "Generator cannot provide more bits than the type can hold");
