...
int cross_compare_internal(struct test* test, xcmp_tag_t tag,
    ...
    if (xcmp->map.contains(tag)) {
        ...
        // so should be the data and we can release the semaphore
        xcmp->sem.release();
...
void *cross_compare_get_expected(struct test *test, xcmp_tag_t tag) {
    auto xcmp = static_cast<xcmp_data_t*>(test->xcmp_data);
    if  (!xcmp) {
        return NULL;
    }
...
int _default_memcmp(void* actual, void* expected, size_t size) {
    return __builtin_memcmp(actual, expected, size);
}
