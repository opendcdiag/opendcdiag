...
int sandstone_main(int argc, char **argv)
{
    ...
    if (opts->enabled_tests.size() || !opts->builtin_test_list_name.empty() || !opts->test_list_file_path.empty()) {
    ...
    if (!opts->builtin_test_list_name.empty()) {
        std::vector<std::string> errors;
        test_set->add_builtin_test_list(opts->builtin_test_list_name.c_str(), errors);
        if (!errors.empty()) {
            // FIXME: handle errors
            ;
            exit(EX_USAGE);
