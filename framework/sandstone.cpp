                                  ...
                                  const PerThreadFailures &per_thread_failures)
{
    ...
    int nfailures = 0;
    for (size_t i = 0; i < num_cpus() && all_threads_failed_equally; ++i) {
    ...
    if (all_threads_failed_equally && nfailures == num_cpus()) {
        logging_printf(LOG_LEVEL_VERBOSE(1), "# All threads failed equally. This is highly unlikely (SW bug?)\n");
        return;
    }

    analyze_test_failures_for_topology(test, per_thread_failures, all_threads_failed_equally);
}
