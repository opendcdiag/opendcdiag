...
static TestResult run_one_test_once(const struct test *test)
{
    ...
    if (device_features_t missing = (test->minimum_device_to_compile | test->minimum_cpu) & ~device_features) {
        init_per_thread_data();
        ...
        device_features_t missing_to_report = missing & ~test->minimum_device_to_compile;
        ...
        if (test->quality_level >= sApp->requested_quality) {
            if (opts.list_tests_include_tests) {
                if (opts.list_tests_include_descriptions) {
                    ...
                    device_features_t feats = test->minimum_device_to_compile & ~device_compiler_features;
