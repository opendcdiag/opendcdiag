...
void sandstone_ssl_init()
{
    ...
    libs.libssl = dlopen("libssl.so.SHLIB_VERSION_NUMBER", RTLD_NOW);
    if (!libs.libssl)
        return;
    libs.libcrypto = dlopen("libcrypto.so.SHLIB_VERSION_NUMBER", RTLD_NOW);
    if (!libs.libcrypto) {
        dlclose(libs.libssl);
        return;
    }
    ...
    auto inner_resolve = [=](const char *name) {
        if (void *ptr = dlsym(libs.libssl, name))
            return ptr;
        return dlsym(libs.libcrypto, name);
    };
#endif
