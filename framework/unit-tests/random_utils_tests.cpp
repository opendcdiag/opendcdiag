...
// The file is included here to have access to the internal state of random_utils, which is essential for testing it.
// These functions should not be visible outside this test suite, therefore random_utils_mock.hpp should not
// have any "internal" declarations.
#include <unit-tests/random_utils_mock.cpp>
...
static void second_mock(void) {
    RandomMock::Values<uint32_t> local_mock{};
    EXPECT_TRUE(local_mock.all_values_used());
}

TEST(RandomUtils, random_mocked) {
    RandomMock::Values<uint32_t> mock{{ 0x01234567U, 0x89abcdefU }};
    ...
    RandomMock::Values<uint64_t> mock64{{ 0x0123456789abcdefULL, }};
    EXPECT_EQ(random64(), 0x0123456789abcdefULL);
}

TEST(RandomUtils, get_random_bits) {
    {
        RandomMock::Values<int> mock_random{};
        EXPECT_EQ(get_random_bits(0), 0); // no bits fetched/consumed!
        EXPECT_DEATH(get_random_bits(1), "");
    }

    RandomMock::Values<int> mock_random{{ 0x12345678, 0x01234567, 0x79abcdef, 0x69696969, 0x5a5a5a5a, 0x54321059 }};
    // first value
...
TEST(RandomUtils, get_random_bits_32) {
    RandomMock::Values<uint32_t> mock{{ 0x01234567, 0x89abcdef, 0xc01dbaad, 0xffffffff }};
    ...
    EXPECT_EQ(get_random_bits_algo(), RandomBitsAlgo::Temporary);
    EXPECT_EQ(get_random_bits_available(), 24);
    // 24 available, 32 fetched. 57 is bigger, then we need to go Assemble path
    EXPECT_EQ(get_random_bits_32(57), 0x1'c01dbaad'89abcd);
    EXPECT_EQ(get_random_bits_algo(), RandomBitsAlgo::Assemble);
    EXPECT_EQ(get_random_bits_available(), 31);
    // some bits will be discarded! Next test should not see them!
}

TEST(RandomUtils, get_random_bits_8) {
    RandomMock::Values<uint32_t> mock{{
        0x12, 0x23, 0x34, 0x45, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88,
        0xff, 0xee, 0xdd, 0xcc,
    ...
    // ff, ee, dd, cc will extend the cache to "fully occupied"
    EXPECT_EQ(get_random_bits_8(32), 0xcc'dd'ee'ff);
    EXPECT_EQ(get_random_bits_algo(), RandomBitsAlgo::ExtendCache);
    EXPECT_EQ(get_random_bits_available(), 0);

    // 33 bits will not fit the cache, we need to assemble the value
    EXPECT_EQ(get_random_bits_8(33), 0x1'66'77'88'99);
    EXPECT_EQ(get_random_bits_algo(), RandomBitsAlgo::Assemble);
    EXPECT_EQ(get_random_bits_available(), 7);

    // the mock is empty
    EXPECT_DEATH(get_random_bits_8(7 + 1), "");
}
...
TEST(RandomUtils, get_random_bits_128) {
    ...
    RandomMock::Values<__uint128_t> mock{{
...
TEST(RandomUtils, get_random_value) {
    ...
    RandomMock::Values<int> mock_random{ mocked_values };
...
TEST(RandomUtils, get_random_value_uniformity) {
    ...
    RandomMock::Counting mock_random{};
    ...
    // Allow 20% for rejections and 2% for distribution variance. LCG is not a very good RNG, so the variance is
    // expected to be higher than for better generators, especially with smaller ranges. With 125k samples per range,
    // the variance should be around 1%, but we allow 2% to be safe.
    static constexpr double ALLOWED_VARIANCE = 1.02;
    static constexpr double ALLOWED_REJECTIONS = 1.20;
    EXPECT_GT(mock_random.get_count(), (NUM_RANDOM_CALLS / ALLOWED_VARIANCE));
    EXPECT_LT(mock_random.get_count(), (NUM_RANDOM_CALLS * (ALLOWED_REJECTIONS * ALLOWED_VARIANCE)));

    for (uint32_t index = 0; index < counts.size(); index++) {
        ASSERT_NE(ranges[index], 0);
        EXPECT_GT(ranges[index], (NUM_VALUES / counts.size()) / ALLOWED_VARIANCE);
        EXPECT_LT(ranges[index], (NUM_VALUES / counts.size()) * ALLOWED_VARIANCE);
