 ...
 */
#ifndef _INCLUDE_GUARD_RANDOM_MOCKER_HPP_
#define _INCLUDE_GUARD_RANDOM_MOCKER_HPP_
...
template<typename T>
struct Values: public IMock<T> {
    Values(void):
        IMock<T>(),
        values(0)
    {}
    Values(std::initializer_list<T> vals):
        IMock<T>(),
        values(vals)
    {}
    Values(const std::vector<T>& vals):
        IMock<T>(),
        values(vals)
    {}
...
#endif // _INCLUDE_GUARD_RANDOM_MOCKER_HPP_
