...
printf "typedef %s cpu_feature_t;\n", $feature_type;
printf "#define CPU_FEATURE_CONSTANT(bit) (((cpu_feature_t) 1) << (bit))\n\n";
...
print q{
static const cpu_feature_t _compilerCpuFeatures = 0};
...
enum X86CpuFeatures : cpu_feature_t {|;
...
enum X86CpuArchitectures : cpu_feature_t {|;
...
for my $state (@xsaveStates) {
    ...
    my $prefix = sprintf "\n// List of features requiring %s%s\nstatic const cpu_feature_t %s%s = 0",
