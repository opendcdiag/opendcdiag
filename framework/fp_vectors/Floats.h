...
struct HFloat8
{
    ...
    constexpr inline bool is_inf_nan() const  { return value == HFLOAT8_NAN_INF_VALUE; }
...
STATIC_INLINE bool HFloat8_is_valid(HFloat8 f)    { return (f.value != HFLOAT8_NAN_INF_VALUE) && (f.value != HFLOAT8_SATURATED_OVERFLOW_VALUE); }
STATIC_INLINE bool HFloat8_is_inf_nan(HFloat8 f)  { return (f.value == HFLOAT8_NAN_INF_VALUE); }
...
enum RANDOM_GEN_FLAGS {
    ...
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_ZERO      = 0x70, // zero, mantissa eq zero implied/forced
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_DENORMAL  = 0x80, // denormal number, 0s excluded (random bit is set to 1 if zero in mantissa)
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_INF       = 0x90, // infinity, zero in the mantissa implied/forced
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_OVERFLOW  = 0xa0, // overflow
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_NAN       = 0xb0, // any NaN, Inf excluded (random bit is set to 1 if zero in mantissa)
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_SNAN      = 0xc0, // signalling NaN, Q bit is cleared
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_QNAN      = 0xd0, // quiet NaN, Q bit is set
    RANDOM_GEN_FLAGS_FLOAT_EXPONENT_MAX       = 0xe0, // max possible exponent, no assumption on the mantissa (Inf/NaN/overflow/large value, depending on the type)
    ...
    VALUE_ZERO = RANDOM_GEN_FLAGS_FLOAT_EXPONENT_ZERO,
    VALUE_INF = RANDOM_GEN_FLAGS_FLOAT_EXPONENT_INF,
    VALUE_DENORMAL = RANDOM_GEN_FLAGS_FLOAT_EXPONENT_DENORMAL,
    VALUE_NAN = RANDOM_GEN_FLAGS_FLOAT_EXPONENT_NAN,
    VALUE_SNAN = RANDOM_GEN_FLAGS_FLOAT_EXPONENT_SNAN,
    VALUE_QNAN = RANDOM_GEN_FLAGS_FLOAT_EXPONENT_QNAN,
...
HFloat8  normalize_hfloat8 (HFloat8 val,  float v1, float v2);
BFloat8  normalize_bfloat8 (BFloat8 val,  float v1, float v2);
Float16  normalize_float16 (Float16 val,  float v1, float v2);
BFloat16 normalize_bfloat16(BFloat16 val, float v1, float v2);
Float32  normalize_float32 (Float32 val,  float v1, float v2);
float    normalize_float   (float val,    float v1, float v2);
Float64  normalize_float64 (Float64 val,  float v1, float v2);
double   normalize_double  (double val,   float v1, float v2);
Float80  normalize_float80 (Float80 val,  float v1, float v2);
...
template<typename T>
inline T normalize_tmpl(T val, float v1, float v2) {
    static_assert(NotImplementedFor<T>::value, "type not handled");
    return val;
}
template<> inline HFloat8 normalize_tmpl(HFloat8 val, float v1, float v2) {
    return normalize_hfloat8(val, v1, v2);
}
template<> inline BFloat8 normalize_tmpl(BFloat8 val, float v1, float v2) {
    return normalize_bfloat8(val, v1, v2);
}
template<> inline Float16 normalize_tmpl(Float16 val, float v1, float v2) {
    return normalize_float16(val, v1, v2);
}
template<> inline BFloat16 normalize_tmpl(BFloat16 val, float v1, float v2) {
    return normalize_bfloat16(val, v1, v2);
}
template<> inline Float32 normalize_tmpl(Float32 val, float v1, float v2) {
    return normalize_float32(val, v1, v2);
}
template<> inline float normalize_tmpl(float val, float v1, float v2) {
    return normalize_float(val, v1, v2);
}
template<> inline Float64 normalize_tmpl(Float64 val, float v1, float v2) {
    return normalize_float64(val, v1, v2);
}
template<> inline double normalize_tmpl(double val, float v1, float v2) {
    return normalize_double(val, v1, v2);
}
template<> inline Float80 normalize_tmpl(Float80 val, float v1, float v2) {
    return normalize_float80(val, v1, v2);
}
...
#define NORMALIZE(val, v1, v2) normalize_tmpl(val, v1, v2)
...
// SET_RANDOM2(val, flags) defined on per-language basis
#define SET_RANDOM(...) OVERLOAD(SET_RANDOM, NARGS(__VA_ARGS__))(__VA_ARGS__)

#define NEW_RANDOM1(type)                  NEW_RANDOM2(type, PATTERNED)
// NEW_RANDOM2(type, flags) defined on per-language basis
// No need to waste time for exponent generation if value will be normalized to given range
#define NEW_RANDOM3(type, v1, v2)        NORMALIZE(NEW_RANDOM2(type, RANDOM_GEN_FLAGS_FLOAT_EXPONENT_BIAS), v1, v2)
#define NEW_RANDOM4(type, flags, v1, v2) NORMALIZE(NEW_RANDOM2(type, flags | RANDOM_GEN_FLAGS_FLOAT_EXPONENT_BIAS), v1, v2)
