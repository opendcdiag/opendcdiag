...
function setup_file() {
    ...
    local t positive_tests=`$SANDSTONE --selftests --list-group-members @positive`
    local -a positive negative
    for t in $kvm_tests; do
        if printf '%s\n' $positive_tests | grep -Fqx "$t"; then
            positive+=($t)
        else
            negative+=($t)
        fi
    done
    export kvm_tests_positive="${positive[@]}"
    export kvm_tests_negative="${negative[@]}"
