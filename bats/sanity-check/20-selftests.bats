...
@test "TAP output @positive" {
    ...
    run $SANDSTONE -n$MAX_PROC --output-format=tap --selftests --quick --timeout=15s $SANDSTONE_DISABLED_TESTS --disable='*timedpass*' -e @positive
    [[ "$status" -eq 0 ]]
    while read line; do
        printf "# %s\n" "$line"
        case "$line" in
...
@test "TAP silent output" {
    local -a opts=(--output-format=tap --quick --selftests --quiet $SANDSTONE_DISABLED_TESTS --disable="*fork" -e @positive)
...
@test "YAML silent output" {
    local -a opts=(-Y --quick --selftests --quiet $SANDSTONE_DISABLED_TESTS --disable="*fork" -e @positive)
...
function selftest_failinit_common() {
    ...
    test_yaml_regexp "/tests/0/fail/$FAILMASK_SYGNATURE" None
...
fail_common() {
    ...
    test_yaml_regexp "/exit" fail
    test_yaml_regexp "/tests/0/result" fail
    test_yaml_regexp "/tests/0/fail/$FAILMASK_SYGNATURE" '[X_.:]+'
    test_yaml_regexp "/tests/0/state/seed" '\w+:\w+'
