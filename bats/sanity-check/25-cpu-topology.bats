...
@test "cpuset=topology (not all match) " {
    ...
    # Get the first logical processor
    local -a cpuinfo=(`$SANDSTONE --dump-cpu-info | sed -n '/^[0-9]/{p;q;}'`)
    run taskset -c 0 $SANDSTONE \
        --cpuset=p${cpuinfo[1]}c${cpuinfo[2]}t${cpuinfo[3]},p${cpuinfo[1]}c${cpuinfo[2]}t3
    [[ $status = 64 ]]
...
@test "selftest_fail_after_reschedule" {
    ...
    test_yaml_numeric "/tests/0/threads/3/previous-cpu/logical" "value == ${cpuset[3]}"
}
...
@test "selftest_fail_after_reschedule_lowest_cpu" {
    ...
    local MAX_PROC=$((MAX_PROC > 64 ? 64 : MAX_PROC))
    local -a cpuset=(`$SANDSTONE -n$MAX_PROC --dump-cpu-info | awk '/^[0-9]/ { print $1 }'`)

    declare -A yamldump
    sandstone_selftest -e selftest_fail_after_reschedule_lowest_cpu --reschedule=queue \
        --max-cores-per-slice=-1    # Disable slicing, so we always get the lowest CPU
    for ((i = 0; i < yamldump[/tests/0/threads@len]; ++i)); do
        # The failing CPU is always the lowest
        test_yaml_numeric "/tests/0/threads/$i/id/logical" "value == ${cpuset[0]}"

        if ((i)); then
            # the previous CPU can't have been the lowest
